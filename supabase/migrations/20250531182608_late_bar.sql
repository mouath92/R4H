...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_allow_public_read" ON users;
  DROP POLICY IF EXISTS "users_allow_public_signup" ON users;
  DROP POLICY IF EXISTS "users_allow_self_update" ON users;
  DROP POLICY IF EXISTS "users_allow_self_delete" ON users;
EXCEPTION
...
CREATE POLICY "allow_signup_with_role_check"
ON users FOR INSERT
TO public
WITH CHECK (
